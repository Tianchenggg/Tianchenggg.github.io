...
test("server-renders the finished research portfolio", async () => {
  ...
  assert.match(html, /Real innovation/);
  assert.match(html, /makes people&apos;s lives easier|makes people&#x27;s lives easier/);
...
test("ships the GitHub Pages export and social assets", async () => {
  const [html, layout, css, page] = await Promise.all([
  ...
  assert.match(page, /\/papers\/rarelens\.png/);
  assert.match(page, /\/papers\/vcu-llm\.png/);
  assert.match(page, /\/papers\/safer-steer\.png/);
  assert.match(page, /\/papers\/safer-toolkit\.png/);
  assert.match(page, /\/papers\/livesearchbench\.png/);
  ...
  await Promise.all([
    ...
    access(new URL("../out/papers/rarelens.png", import.meta.url)),
    access(new URL("../out/papers/vcu-llm.png", import.meta.url)),
    access(new URL("../out/papers/safer-steer.png", import.meta.url)),
    access(new URL("../out/papers/safer-toolkit.png", import.meta.url)),
    access(new URL("../out/papers/livesearchbench.png", import.meta.url)),
